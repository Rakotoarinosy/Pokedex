    ...
    fetch("https://pokeapi.co/api/v2/pokemon?limit=151")
        ...
        .then((allPoke) =>{
            ...
            allPoke.results.forEach((pokemon)=> {
                fetchPockemonComplet(pokemon);
            });
        })
}
fetchPockemonBase()

function fetchPockemonComplet(pokemon){
...
searchInput.addEventListener('input', function(e){
    if (e.target.value !== "") {
        e.target.parentNode.classList.add('activate-input');
    } else if(e.target.value === ""){
        e.target.parentNode.classList.remove('activate-input');
    }
})